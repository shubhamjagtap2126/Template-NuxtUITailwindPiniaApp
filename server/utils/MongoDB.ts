...
export const getModel = (key: string) => {
  const model = models.find((model) => String(model.modelName) === key);
  if (!model) {
    throw createError({
      statusCode: 400,
      statusMessage: `Collection logic for '${key}' is not defined in Registry.`,
    });
  }
  return model;
};
...
import { uid } from 'uid';
export const generateUID = async (schema, len = 6) => {
  schema.add({ _id: { type: String, default: () => uid(len) } });

  schema.pre('save', async function (next) {
    if (!this.isNew) return next();

    for (let i = 0; i < 10; i++) {
      if (!(await this.constructor.exists({ _id: this._id }))) return next();
      this._id = uid(len);
    }

    next(new Error('UID: max retries exceeded'));
  });
};
 ...
 */
export const mapFilter = (conditions) => {
  const query = {};
...
export const bulkInsert = async (Model, dataArray) => {
  const chunks = Array.from({ length: Math.ceil(dataArray.length / BATCH_SIZE) }, (_, i) => dataArray.slice(i * BATCH_SIZE, i * BATCH_SIZE + BATCH_SIZE));
 ...
 */
export const streamRead = async (Model, conditions, options = {}) => {
...
export const bulkUpdate = async (Model, updates) => {
  ...
  const chunks = Array.from({ length: Math.ceil(operations.length / BATCH_SIZE) }, (_, i) => operations.slice(i * BATCH_SIZE, i * BATCH_SIZE + BATCH_SIZE));
...
export const runAggregation = async (Model, pipeline = [], options = {}) => {
  ...
  try {
    const results = await Model.aggregate(pipeline).option({ ...defaultOptions, ...options });
 ...
 */
export const streamAggregation = async (Model, pipeline = [], options = {}) => {
...
/**
 * Recursively converts UTC dates (ending in 'Z') to Local Time String (+5:30)
 * Works for both Single Objects (findById) and Arrays (find)
 */
...
export const formatResponseDates = (data) => {
  ...
  if (Array.isArray(data)) {
    return data.map((item) => formatResponseDates(item));
  }

  if (typeof data === 'object' && data !== null) {
    if (data._bsontype) return data;

    return Object.fromEntries(
      Object.entries(data).map(([key, value]) => {
        const isDateString = typeof value === 'string' && value.endsWith('Z');
        const isDateObject = value instanceof Date;

        if (isDateString || isDateObject) {
          const adjustedDate = dateFormatter.format(new Date(value));
          return [key, adjustedDate.replace(',', '')];
        }

        if (typeof value === 'object' && value !== null) {
          return [key, formatResponseDates(value)];
        }
